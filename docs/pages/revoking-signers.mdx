...
Wallet clients will typically not replay revocations on known chains where the wallet has no assets or activity. In addition to those chains, there will always be new chains that launch after a signer is compromised.
...
Once one of these paths are viable, we expect to change our recommendation to require a recent sync even before setConfig calls can be used, which would limit the window of time that a compromised signer can be used to the eventual consistency period plus the settlement delay of the master chain.
